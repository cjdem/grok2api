...
const LOCALE_MAP = {
  ...
  "grok": {
    ...
    "show_search": { title: "搜索过程", desc: "是否在思维链中展示搜索查询与结果计数。" },
    ...
    "conversation_enable_native": { title: "原生会话续聊", desc: "启用后将使用 Grok 原生 conversation/response 链路保持上下文。" },
    "conversation_ttl_seconds": { title: "会话保留秒数", desc: "会话在 D1 中的保留时长，过期后自动清理。" },
    "conversation_max_per_token": { title: "单 Token 会话上限", desc: "每个 scope+token 最多保留的会话数，超过会淘汰最旧会话。" },
    "conversation_sticky_token": { title: "会话粘性 Token", desc: "续聊优先复用历史 token；不可用时自动回退并尝试克隆会话。" },
